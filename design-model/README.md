...
案例二：
手机操作：有三种不同款式的手机：折叠、直立、旋转；又有三种不同品牌的手机：华为、vivo、小米；每个款式都各有一个品牌的手机、要求都可以打电话
...
###十：原型模式
####1：概念
用原型实例指定创建对象的种类，并且通过拷贝这些原型创建新的对象：通过对象.clone实现
####2：目的
####3：角色
原型抽象类：声明一个克隆自己的方法
具体原型类：实现克隆自己的方法
...
版本一：使用传统方式实现：在创建对象的时候需要初始化对象，而不是动态获取对象运行时状态
版本二：采用原型模式实现
二：
考试试卷：实现一个上机考试抽题的服务，建造一个题库题目的场景类信息，用于创建；选择题、问答题。
版本一：使用传统方式实现
...
####4：案例
一：模拟学生和老师不同视角访问
对于家长而言：他更多关注的是孩子的排名和老师的能力；
对于校长而言：他更多关注的是学生的升学率以及学生数量
