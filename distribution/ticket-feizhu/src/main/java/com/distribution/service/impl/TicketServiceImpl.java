    ...
    private String buyTicketVersion7(Long userId) {
        ...
        try {
            ...
            if (stock > 0) {
                ...
                System.out.println("【飞猪旅行】：尊敬的" + userId + "号旅客：恭喜您抢票成功，您的票号为" + stock + "还剩" + realStock + "张票");
                return "【飞猪旅行】：尊敬的" + userId + "号旅客：恭喜您抢票成功，您的票号为" + stock + "还剩" + realStock + "张票";
            } else {
        ...
        try {
            ...
            if (lock) {
                if (number > 0) {
                    ...
                    System.out.println("【飞猪旅行】：" + Thread.currentThread().getName() + "抢到了" + num + "号票，" + "还剩" + finaCount + "张票");
                    resMsg = "抢票成功...";

                    // 将票数重新设置到redis中
                    this.redisTemplate.opsForValue().set(RedisPathEnum.TICKET_STOCK.path,finaCount.toString());
                }else {
                    System.out.println("【飞猪旅行】：暂无余票");
                    resMsg = "暂无余票...";
                }
            }
            // 释放锁
            ...
            System.out.println("【飞猪旅行】：很遗憾：" + Thread.currentThread().getName() + "抢票失败");
